  ...
  void do_execute_and_check_binning(const int nSpectra, const int pulseTimeMin,
    ...
    for (int i = 0; i < nSpectra; ++i) {
      ...
      for (uint64_t j = 0; j < Y.size(); ++j) {
        ...
        // Should have 1 event per bin, because that's what the
        // createEventWorkspace() provides and our rebinning params are based on
        // our original creation parameters.
      }
    }
  }
...
public:
  ...
  void test_zero_step_throws() { do_test_bad_step_throws(0); }

  void test_less_than_zero_step_throws() { do_test_bad_step_throws(-1); }
...
// Performance Tests
//=====================================================================================
template <class AlgorithmType> class RebinByTimeBaseTestPerformance {
private:
