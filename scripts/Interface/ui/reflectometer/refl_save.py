...
# file format selector
        ...
        self.pushButton.clicked.connect(self.buttonClickHandler1)
        self.pushButton_2.clicked.connect(self.populateList)
        self.lineEdit.textChanged.connect(self.setPath)
        self.filterEdit.textChanged[str].connect(self.filterWksp)
        QtCore.QObject.connect(self.listWidget, QtCore.SIGNAL(_fromUtf8("itemActivated(QListWidgetItem*)")), self.workspaceSelected)
     #   self.actionSave_table.triggered.connect(self.saveDialog)
     #   self.actionLoad_table.triggered.connect(self.loadDialog)
